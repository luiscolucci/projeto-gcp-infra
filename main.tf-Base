...
# ==========================================
provider "google" {
  project = "NOME PROJETO" # Ex: projeto-gcp-prod
  region  = "us-central1"
  user_project_override = true
  billing_project       = "NOME PROJETO"
}
...
resource "google_monitoring_notification_channel" "alerta_email" {
  ...
  labels = {
    email_address = "seuemail@email.com" # <--- MUDE AQUI
  }
}
...
resource "google_monitoring_notification_channel" "alerta_sms" {
  ...
  depends_on   = [google_project_service.api_monitoring]
  labels = {
    number = "+5511999999999" # <--- MUDE AQUI (Ex: +5511999999999)
  }
}
...
resource "google_billing_budget" "alerta_custos" {
  billing_account = "xxxxxx-xxxxxx-xxxxxx" # <--- MUDE AQUI (Ex: XXXXXX-XXXXXX-XXXXXX)
...
resource "google_monitoring_uptime_check_config" "apache_uptime" {
  ...
  monitored_resource {
    type = "uptime_url"
    labels = {
      project_id = "ID PROJETO" # <--- MUDE AQUI (Igual ao do Bloco 1)
